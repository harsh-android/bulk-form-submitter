...
chrome.runtime.sendMessage({ type: 'captureScreenshot' }, (resp) => {
    if (resp && resp.ok) chrome.runtime.sendMessage({ type: 'screenshotResult', ok: true, dataUrl: resp.dataUrl });
    else chrome.runtime.sendMessage({ type: 'screenshotResult', ok: false, error: resp && resp.error });
});
